...
// --- FUNÇÃO PARA ENVIAR PEDIDO PENDENTE PARA WHATSAPP ---
function enviarPedidoPendenteParaWhatsApp(pedidoData) {
    console.log('📤 Enviando pedido pendente para WhatsApp:', pedidoData.cliente);
    
    // URL para o servidor WhatsApp
    const whatsappUrl = 'http://localhost:3001/enviar';
    ...
    .then(response => {
        console.log('📨 Resposta do servidor WhatsApp:', response.status, response.statusText);
        if (!response.ok) {
    ...
    .then(data => {
        ...
        if (data.status === 'ok') {
            console.log('🎉 Pedido pendente enviado para WhatsApp com sucesso!');
        } else {
            console.error('❌ Erro no envio WhatsApp:', data.message);
        }
    })
    .catch(error => {
...
auth.onAuthStateChanged(user => {
    if (user) {
        ...
        pedidosRef.on('child_added', (snapshot) => {
            ...
            renderizarPedido(pedidoData, pedidoId, false);
            resetInactivityTimer();
            atualizarContadorPendentes();
            atualizarContadorConfirmados();
            
            // Envia novo pedido pendente para WhatsApp
            console.log('📱 Novo pedido recebido na central - enviando para WhatsApp:', pedidoData.cliente);
            enviarPedidoPendenteParaWhatsApp(pedidoData);
        });
...
document.querySelector('main').addEventListener('click', (event) => {
    ...
    if (target.classList.contains('confirmar-btn')) {
        ...
        // WhatsApp já foi enviado quando o pedido foi criado (pendente)
        console.log('✅ Pedido confirmado - WhatsApp já foi enviado anteriormente');
    ...
    pedidoDiv.innerHTML = `
        ...
        <div class="pedido-total-acoes">
            ...
            <div class="button-container">
                ...
                <div class="whatsapp-status" title="Enviado para WhatsApp">
                    ...
                    <span class="whatsapp-text">Enviado</span>
                </div>
            </div>
        </div>
    `;
